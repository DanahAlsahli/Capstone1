...
public class UserController {
    ...
    public ResponseEntity<?> buyProduct(@PathVariable int userId, @PathVariable int productId, @PathVariable int merchantId) {
        ...
        return ResponseEntity.status(400).body(new ApiResponse("can not buy product"));
    }
    ...
    public ResponseEntity<?> addBalance(@PathVariable int userId, @PathVariable double amount) {
        ...
        return ResponseEntity.status(400).body(new ApiResponse("user not found or amount is not valid"));
    }
    ...
    public ResponseEntity<?> withdrawBalance(@PathVariable int userId, @PathVariable double amount) {
        ...
        boolean withdrawn = userService.withdrawBalance(userId, amount);

        if (withdrawn) {
            return ResponseEntity.status(200).body(new ApiResponse("balance withdrawn"));
        }

        return ResponseEntity.status(400).body(new ApiResponse("user not found or balance is not enough"));
    }
    ...
    public ResponseEntity<?> recommendProduct(@PathVariable int userId) {
        ...
        if (product == null) {
            return ResponseEntity.status(400)
                    .body(new ApiResponse("no product available"));
        }
